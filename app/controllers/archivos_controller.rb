 ...
 def subir_archivos
    if  user_signed_in?
      ...
      if request.post?
         ...
         nombre = "E2R9U3HN9W"+(@current_user.tipo_id).to_s+"WFEOEKBi"+(params['id']).to_s+extension;
         if extension == ".pdf" or extension == ".doc" or extension == ".docx"
            ...
            if resultado
               ...
               #subir_archivo = "error";
               redirect_to inicio_path , alert: 'Error al subir archivo'
            end
              redirect_to inicio_path, notice: 'se subio el archivo correctamente'
           ...
           redirect_to inicio_path, alert:'El formato del archivo ingresado es incorrecto. Usted sólo puede subir archivos .pdf, .doc y .docx'
